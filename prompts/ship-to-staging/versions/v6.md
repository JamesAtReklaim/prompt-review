...
**Stage check first:** if the change on your current branch is ALREADY merged and
live on staging, this prompt's sync/PR/merge machinery is moot — do NOT re-run it.
Skip straight to the `staging-finalise` prompt for closeout.
...
3. **Auto-merge to staging by default — pause only on a real blocking reason.**
   ...
   explicit "merge it". This auto-merge default is an INTENTIONAL, staging-only
   override of `pr-lifecycle.mdc` Phase 5's "merge only on explicit
   authorization" — staging is reversible via revert, so the "docs win" rule does
   NOT cancel it here; it's a deliberate exception for staging, not a conflict to
   resolve in the docs' favour. It does NOT extend to production.
   ...
   - (b) a review (Copilot / Bugbot / human) surfaced an unresolved, valid
     concern;
   - (c) the change touches a risky surface (auth, payments, migrations, data
     deletion / PII, security / RLS) **AND** that touch still carries an actual
     unresolved concern — a real bug/regression (a), an open valid review finding
     (b), a red/unexplained gate (d), or something here you are genuinely not
     confident in or think may be dangerous. A risky surface ALONE is NOT a reason
     to pause: on a clean run — all required gates green, both review rounds clean,
     no unresolved finding or bug — auto-merge it like any other change, but
     explicitly note in the report that it touched a risky surface and why it's
     safe (which gates/proofs cover it). (We always check risky surfaces
     thoroughly before this point; the pause is for when that checking actually
     surfaces something to resolve, not for the mere fact of touching the surface.)
...
5. **CI pitfalls not in the docs (avoid them):**
   ...
   - **Renaming a migration file after the first push** orphans the old version
     in the per-PR Supabase preview branch, surfacing as a red ❌ "Remote
     migration versions not found in local migrations directory." This is the
     SAME benign preview artifact as above — NOT the authoritative
     `Deploy to Supabase Staging` job and NOT a real migration failure, though it
     looks alarming. Prefer finalising the migration timestamp BEFORE the first
     push; if you must rename post-push, expect this preview ❌ and pre-explain it
   ...
   - Check-status tooling can be permission-blocked: `gh pr checks <N>` (and the
     REST `commits/<sha>/check-runs` endpoint) can 403 with "Resource not
     accessible by personal access token" when the token lacks `checks:read`.
     Fall back to `gh run list --branch <b>` + `gh run watch <run-id>`, and/or
     the GitHub MCP `get_status`. Also: `gh run watch … --exit-status` can exit
     NON-ZERO on a dropped connection while the run is still `in_progress` — a
     false failure. Re-query `gh run view <id> --json status,conclusion` before
     treating any non-zero watch exit as a real failure.

6. **Review-round stop rule (don't loop on cosmetic churn).** Copilot/Bugbot
   rounds can balloon, and a "fix" often introduces a fresh trivial doc-nit that
   gets re-flagged the next round. Resolve all SUBSTANTIVE findings, but once a
   round yields only cosmetic/doc nits (or nits re-introduced by your previous
   fix), batch-resolve them and stop. The gate is "two consecutive rounds with no
   new SUBSTANTIVE findings," not "zero comments" — don't chase a clean slate of
   nits.
