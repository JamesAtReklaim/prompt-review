...
- `.cursor/rules/qa-checklist.mdc` — the QA Notion page + Sign-Off (precondition
  for Ready to Deploy).
...
1. **Post a proof comment on the Linear issue** demonstrating the problem is
   ...
   - **Visual evidence is mandatory whenever ANY acceptance criterion is visual
     ...
     criterion.
   - **Authenticated visuals you can't reach:** if the visual proof lives behind
     login and you don't already have the bytes, do NOT burn rounds on a fragile
     headless OTP login — make "ask me to paste the screenshot into a Linear
     comment (or attach the file)" the FIRST-line option for authenticated visuals,
     not a last resort. Then download those bytes, re-upload them under your OWN
     issue asset (the embed flow below), embed THAT, and verify it renders.
   - **Clean up the hand-off:** once your own-asset embed renders, fold the image
     into the consolidated proof comment and then DELETE the now-redundant hand-off
     comment — but re-host the image under your own asset FIRST so the deletion can't
     break the embed. Do this proactively, without being asked.
...
3. **Update the QA Notion Sign-Off (precondition for Ready to Deploy).** Per
   `.cursor/rules/qa-checklist.mdc`, a completed Sign-Off is REQUIRED before the
   status moves to Ready to Deploy. Fill the QA page's Sign-Off row
   (Tester / Date / Result / Notes) and tick the verified items. Don't skip this
   or wait to be nudged — it's the precondition for step 4, not an optional extra.
4. **Set the Linear status to `Ready to Deploy`** (only after the QA Sign-Off in
   step 3 is complete) — per `.cursor/rules/slack-and-linear.mdc`, that's the
   verified-on-staging end-state. NEVER set **Done** (production-only, out of
   scope here). My "I'm happy" sign-off is the explicit instruction that
   authorizes this otherwise integration-owned transition.
