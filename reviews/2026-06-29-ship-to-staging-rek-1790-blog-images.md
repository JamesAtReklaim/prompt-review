...
- [ ] [ship-to-staging] The Linear proof comment **omitted images even though part of the acceptance was a visual check** (blog still renders on staging), and the director had already supplied staging screenshots in-chat. The agent classified the change as "backend/RLS" and used text-only proof (the `403` + DB query), dropping the visual evidence entirely. The finalisation step's "pick whatever genuinely proves it" let the agent under-include: a backend change with a visual regression check is not purely "backend," and the visual proof was silently skipped instead of the agent capturing it (public surface) or asking the director for the images it already had.
...
- [ ] [ship-to-staging] In the finalisation/proof step, make visual evidence mandatory whenever ANY acceptance criterion is visual — even on an otherwise-backend change. The agent must include images of the **deployed-staging** surface (desktop + mobile if UI), and if it does not already have them it must EITHER capture them (public surface via the browse CLI) OR explicitly ask the director for the images before posting — never post a proof that drops a visual criterion. Add a pre-post checklist line: "Does any acceptance item involve what something looks like? If yes, the comment must contain matching images." (This complements, not replaces, the existing "verify the comment renders after posting" rule.)
